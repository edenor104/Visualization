...
for participant_folder in participant_folders:
    participant_folder_path = os.path.join(combined_data_path, participant_folder)

    # Get a list of files in the participant folder
    participant_files = os.listdir(participant_folder_path)

    # Find the files with 'Summary' and 'RawData' in their names
    summary_file = [file for file in participant_files if 'Summary' in file]
    raw_file = [file for file in participant_files if 'RawData' in file]

    if summary_file and raw_file:
        participant_name = participant_folder
        raw_file_path = os.path.join(participant_folder_path, raw_file[0])
        raw_data = pd.read_csv(raw_file_path)
        raw_data['Participant'] = participant_name
        raw_data_list.append(raw_data)

# Concatenate the data in the lists to create the dataframes
...
def update_graph(selected_maze_type, selected_condition_string, selected_participant):
    ...
    if selected_condition_string != None:
        selected_condition = condition_conversion[selected_condition_string]
    else:
        selected_condition = selected_condition_string
